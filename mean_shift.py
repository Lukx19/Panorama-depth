...
"""This code originates from
    https://github.com/svip-lab/PlanarReconstruction/blob/master/bin_mean_shift.py
    If ussing please cite:
...
class Bin_Mean_Shift(nn.Module):
    ...
    def generate_seed(self, point, bin_num):
        ...
        device = point.get_device()

        start_x, end_x = get_start_end(min_x.item(), max_x.item(), bin_num)
        start_y, end_y = get_start_end(min_y.item(), max_y.item(), bin_num)

        x = torch.linspace(start_x, end_x, bin_num, device=device).view(bin_num, 1)
        y = torch.linspace(start_y, end_y, bin_num, device=device).view(1, bin_num)
    ...
    def label2onehot(self, labels):
        ...
        onehot = torch.zeros((n, label_num), device=labels.get_device())
